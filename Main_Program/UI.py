...
sort_process = subprocess.Popen(['python', '../small_pool/sorting_microservice/sort_microservice.py' ])
limit_process = subprocess.Popen(['python', '../big_pool/result_limiting_microservice/results_limit_microservice.py'])
format_process = subprocess.Popen(['python', '../big_pool/format_data_display_microservice/display_table.py'])
search_process = subprocess.Popen(['python', '../big_pool/keyword_search_microservice/search_microservice.py'])
time.sleep(2)
...
# create socket for formatting microservice
format_socket = context.socket(zmq.REQ)
format_socket.connect("tcp://localhost:5565")

# create socket for search microservice
search_socket = context.socket(zmq.REQ)
search_socket.connect("tcp://localhost:5552")
...
# -----------------------------------------------------------
# Function: prints title search menu
# Receives: None
# Returns: None
# -----------------------------------------------------------
def search_menu():
    print(' You can search the titles of your results by entering a keyword to search for.')
    print(' Type your keyword and press enter. Any titles that have the matching keyword will be displayed.')
    print(' You may also use the optional commands below.')
    print()
    print('    0 to Exit Program')
    print('    1 to Proceed')
    print('    2 to go Back')
...
while running:
    ...
    running = results_screen(results, format_socket)
    ...
    # Sort Screen
    # ------------------------------------------------------------------------
    while running == '12' or running =='13' or running == '14':
        ...
        if running == '12':
            ...
            while not sort_valid:
                ...
                if sort_input == '1' or sort_input == '':
                    sort_attr[0] = original_results.to_json()
                    sort_attr[1] = 'Name'
                elif sort_input == '0':
                    # user chose to exit
                    time.sleep(1)
                    ...
                    original_results['Metacritic_Rating'] = pd.to_numeric(original_results['Metacritic_Rating'],
                    errors='coerce')
                    sort_attr[0] = original_results.to_json()
        ...
        # ------------------------------------------------------------------
            ...
            # process user input for search screen
            if keyword_input == '0':
                exit_app()
            elif keyword_input == '2':
                running = results_screen(results, format_socket)
            elif keyword_input == '1' or keyword_input == '':
                running = results_screen(results, format_socket)
            else:
                payload = json.dumps ({"command": "find", "key": "Name", "keyword": keyword_input, "dataset": results.to_dict(orient='records')})
                search_socket.send_string(payload)
                response = json.loads(search_socket.recv_string())

            if response.get("status") == "success":
                search_results = pd.DataFrame(response["results"])
                if search_results.empty:
                    print()
                    print('   No titles found matching your keyword.')
                    running = results_screen(search_results, format_socket)
                else:
                    running = results_screen(search_results, format_socket)
            else:
                print()
